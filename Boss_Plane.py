...
class GameView(arcade.View):
    ...
    def setup(self):
        ...
        self.boss_list = arcade.SpriteList(use_spatial_hash=True)
    ...
    def reset_map(self, new_map):
        ...
        self.boss_list = arcade.tilemap.process_layer(my_map, boss_layer_name, TILE_SCALING * 2.25)
    ...
    def on_update(self, delta_time):
        ...
        if arcade.check_for_collision_with_list(self.player_sprite, self.thunder_list) or arcade.check_for_collision_with_list(self.player_sprite, self.bullet_list) or arcade.check_for_collision_with_list(self.player_sprite, self.boss_bullet_list):
            death_sound = arcade.load_sound("Sounds\Death.mp3")
        ...
        if arcade.check_for_collision_with_list(self.player_sprite, self.finishing_line_list):
            end_of_level = arcade.load_sound("Sounds\EndOfLevel.mp3")
            arcade.play_sound(end_of_level)
            self.player_sprite.change_x = 0
            self.player_sprite.change_y = 0
            self.player_sprite.center_x = player_start_x
            self.player_sprite.center_y = player_start_y
        ...
        for player_bullet in self.player_bullet_list:
            ...
            hit_list = arcade.check_for_collision_with_list(player_bullet, self.boss_list)
            if len(hit_list) > 0:
                player_bullet.remove_from_sprite_lists()

            for boss in hit_list:
                boss.remove_from_sprite_lists()
                hit_sound = arcade.load_sound("Sounds\Ding.mp3")
        ...
        for boss in self.boss_list:
            ...
            if math.sqrt((x_diff * x_diff) + (y_diff * y_diff)) < 700:
                ...
                if self.frame_count % 60 == 0:
                    boss_bullet = arcade.Sprite("Sprites\Boss\Boss_Bullet.png")
                    boss_bullet.center_x = start_x
                    boss_bullet.center_y = start_y
                    boss_bullet.angle = math.degrees(angle) - 90

                    boss_bullet.change_x = math.cos(angle) * BULLET_SPEED * 5
                    boss_bullet.change_y = math.sin(angle) * BULLET_SPEED * 5

                    self.boss_bullet_list.append(boss_bullet)

        for boss_bullet in self.boss_bullet_list:
            if math.sqrt(((self.player_sprite.center_x - boss_bullet.center_x) * (
                    self.player_sprite.center_x - boss_bullet.center_x)) + (
                                 (self.player_sprite.center_y - boss_bullet.center_y) * (
                                 self.player_sprite.center_y - boss_bullet.center_y))) > 800:
